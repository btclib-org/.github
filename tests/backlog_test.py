...
rather than the organization. A row of `tests/repositories.py` excuses
a failure the tracker records, as a strict expected failure, so that a
repository that catches up is reported until its name is taken out. A
repository the run skips instead -- the file the test reads gone from
the tree, its tier moved past the one the test asks -- would be reported
as neither, and the row would go on excusing a cell nobody asks.
