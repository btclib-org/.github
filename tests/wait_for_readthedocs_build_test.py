...
# the two answers each of those cases scripts before the one that
# ends the wait -- the page in the first, the deadline in the second
_ANSWERS_BEFORE_THE_PAGE = 2
_ANSWERS_BEFORE_THE_DEADLINE = 2
...
def test_a_build_that_has_not_finished_is_waited_for(
    ...
    assert reported.count(f"{_URL} answered HTTP 404") == _ANSWERS_BEFORE_THE_PAGE
...
def test_a_build_that_never_arrives_is_the_deadline_speaking(
    ...
    assert len(site.asked) == _ANSWERS_BEFORE_THE_DEADLINE
    reported = capsys.readouterr().out
